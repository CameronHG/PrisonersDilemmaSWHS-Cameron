    ...
    def myprisoners_dilemma():
        ...
        def play_single_dilemma(player_1: Team,
            ...
            if player_1_move not in GLOBALS.ACCEPTABLE_RESPONSES and not player_1_move == 'n':
                ...
                player_1_move = 'n'
                print(
                    f"{player_1.team_name} made a bad response in a match against {player_2.team_name}, deducting {GLOBALS.CRASH * -1} points as a result")
            if player_2_move not in GLOBALS.ACCEPTABLE_RESPONSES and not player_1_move == 'n':
                player_2_round_score = GLOBALS.CRASH
                player_2_move = 'n'
                print(
                    f"{player_2.team_name} made a bad response in a match against {player_1.team_name}, deducting {GLOBALS.CRASH * -1} points as a result")
        ...
        def display_standings(teams: List[Team], scores: List[List[int]]):
            ...
            for team_index in range(len(teams)):
                ...
                print('{0:2}) {1:<16}(P{2}): {3:>8} points with {4:<20}'.format(team_index + 1, team.team_name,
                                                                                team.team_number, team.summed_scores,
                                                                                team.strategy_name))
